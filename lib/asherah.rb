...
module Asherah
  ...
  def self.setup(
    ...
    raise "Setup failed: #{result}" if result.negative?
  end

  def self.int64_to_buffer(number)
    buffer_ptr = FFI::MemoryPointer.new(1, Cobhan::BUFFER_HEADER_SIZE, false)
    buffer_ptr.put_int64(0, number)
    buffer_ptr
  end

  def self.buffer_to_int64(buffer_ptr)
  ...
  def self.encrypt(partition_id, data)
    ...
    raise "Encrypt failed: #{result}" if result.negative?
  ...
  # @return [String]
  def self.decrypt(partition_id, data_row_record)
    partition_id_buffer = string_to_cbuffer(partition_id)
    ...
    raise "Decrypt failed: #{result}" if result.negative?
